...
if config:
    ...
    goodchips = goodchips_dict[instru][target][band]
    if simulation_on and instru == "IGRINS":
        goodchips = [1, 2, 3, 4] if band == "K" else [3,4,5,6,18] # "H"
    nchip = len(goodchips)
...
mean_spectrum, template, observed, residual, error = load_data(model_datafile, instru, nobs, goodchips, use_toy_spec)
...
# Solve by 5 solvers
bestparamgrid_r, bestparamgrid = solve_IC14new(intrinsic_profiles, obskerns_norm, goodchips, kwargs_IC14, kwargs_fig, annotate=True)

LSDlin_map = solve_LSD_starry_lin(intrinsic_profiles, obskerns_norm, goodchips, kwargs_run, kwargs_fig, annotate=True)
...
lin_map = solve_starry_lin(mean_spectrum, observed, goodchips, kwargs_run, kwargs_fig, annotate=True)
#plt.figure(figsize=(5,3))
#plt.savefig(paths.figures / f"{savedir}/solver4.pdf", bbox_inches="tight", dpi=300)

opt_map = solve_starry_opt(mean_spectrum, observed, nchip, kwargs_run, kwargs_fig, lr=lr, niter=2000, annotate=True)
